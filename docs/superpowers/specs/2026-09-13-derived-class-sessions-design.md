...
Today, a `schedule`-type class's occurrences are pre-generated: `POST /classes/{id}/generate-occurrences`
bulk-inserts up to 104 concrete `sessions` rows upfront (`docs/superpowers/specs/2026-09-12-operator-scheduling-design.md`).
This has two real costs: an operator must remember to keep calling `generate-occurrences` or a class silently
runs out of bookable dates, and any class-level edit (rename, capacity change) that should apply to future
...
- Remove `generate-occurrences` and the 104-occurrence cap entirely. Future/past occurrence lists are computed
  from the class's pattern within an explicit, caller-supplied date range (full calendar days, capped at 90
  days per call).
- Materialize a real `sessions` row for a date only when it diverges from the plain pattern: reschedule,
  cancel, or attendance recorded for it (including a nightly backfill job, described below).
...
- Add a nightly in-process job that backfills any class's missing materialized sessions from the day after its
  last materialized/attendance-bearing session through yesterday (inclusive), self-healing if a run is missed.
- Keep makeup sessions exactly as they work today (explicit, always-materialized, roster auto-filled from the
  class's current standing members), merged into the future-occurrences list so a calendar view sees them
  alongside regular derived dates.
- Leave `assigned`-type operators' flow (`POST /api/operator/sessions`, `/sessions/{id}/reschedule`,
  `/sessions/{id}/cancel`) completely untouched — this spec only changes `schedule`-type class derivation.

## Non-goals

- No changes to `assigned`-type operators' one-off session flow, or to the existing `assign-students`/
  `unassign-students` endpoints' semantics (only their pause-check condition changes, to `status !== 'stopped'`).
...
- `status: 'active' | 'stopped'`. Stopping sets both `status: 'stopped'` and `stopped_at: NOW()`. Unstopping
  sets `status: 'active'` and clears `stopped_at` to `NULL`.
...
- `class_id` is denormalized (same pattern as `enrollments_and_credits.household_id` elsewhere in this schema)
  so attendance can be queried/reported per class without joining through `sessions`.
...
Same shape as `session_attendance` plus `archived_at`; FKs kept (per explicit decision) rather than decoupled,
so historical rows remain referentially valid even though they're rarely queried once archived.

### `sessions` (no schema change)

Unchanged. Still the home for: `assigned`-type one-off sessions (as today), makeup sessions (as today,
`is_makeup_session: true`), and now also every materialized `schedule`-type occurrence — whether materialized
by an explicit reschedule/cancel/attendance action, or by the nightly backfill job.
...
- **`GET /classes/{id}/occurrences/future?from=&to=`** — `from` must be today-or-later; `to - from` ≤ 90 days.
  ...
  row (no `id`; `classId` + computed `startTime`), (b) already-materialized `sessions` rows in range (a past
  reschedule may have moved a date into or out of this window), and (c) makeup sessions in range
  (`isMakeupSession: true`). Every non-makeup entry also carries `classMemberStudentIds` (the class's *current*
  standing roster) so a calendar view can show "who's enrolled" without a second call — reading the roster live
  means a virtual future date is always in sync with roster changes, with nothing to propagate.
...
- `PATCH /classes/{id}/occurrences/{date}/reschedule` — materializes the date (with the pattern's default
  `startTime`) and then immediately applies the requested new `startTime` to that same row, in one call.
...
new deployment surface). Once nightly, for every non-stopped **`schedule`-type** operator's classes only — the
job explicitly excludes `assigned`-type classes (the 1:1 `maxSize: 1` variant from the original
2026-09-12 spec): those represent a single ongoing 1:1 relationship, not a derived weekly pattern meant to
produce attendance-trackable occurrences, and this whole spec's derivation/attendance model is scoped to
`schedule`-type classes only, consistent with every other section above. For each qualifying class: find the
latest date it has a materialized `sessions` row for (or the class's creation date if it has none yet), and
materialize every derived occurrence date after that point through **yesterday inclusive** — even if that's
several weeks of dates (e.g. the process was down for a while), so attendance can always be reported/corrected
for every past date via `GET .../occurrences/past` and `PUT .../attendance`. This is the only place `sessions`
rows are created without a specific human action prompting that exact date.
...
sessions to allow setting missing ones" and "allow updating attendance status for students."
...
materialized `sessions` row (`isMakeupSession: true`), with the attendee list **auto-filled from the class's
current `class_enrollments` standing roster** at creation time (not editable per-student at creation — this
reverses the original 2026-09-12 spec's "operator explicitly picks students" design, superseded by this spec's
explicit confirmation that a makeup should default to the same roster as the class it's making up for, matching
the "vacation day, recovered next week" framing). It is not part of the weekly-pattern derivation itself; it's
an explicit exception layered on top, exactly as before. The only change is that `GET .../occurrences/future`
now includes it merged into the same response as regular derived/materialized dates, tagged `isMakeupSession`
so a calendar UI can render it distinctly (e.g. "Sat Sept 20 — Makeup").
...
- `POST /classes/{id}/assign-students`, `/unassign-students` — same semantics; their "class must not be
  paused" guard becomes "class must not be stopped."
...
- `POST /api/operator/sessions`, `PATCH /sessions/{id}/reschedule`, `POST /sessions/{id}/cancel` —
  completely untouched; this is the `assigned`-type operator's one-off/1:1 booking flow, and was never part of
  the class-derivation model. These numeric-id-addressed endpoints also continue to work for any
  already-materialized `schedule`-type session, alongside the new date-addressed variants.
- `GET /classes/{id}` / `GET /classes?operatorId=` / `POST /classes` / `PUT /classes/{id}` / `DELETE /classes/{id}`
  — unchanged, except `PUT`'s `renameRelatedSessions` flag now only needs to rename *already-materialized*
  future sessions (as it does today); virtual/unmaterialized dates always read the class's current title live,
  so they need no propagation at all — this shrinks, rather than removes, that flag's relevant surface.
- `GET /api/operator/sessions?operatorId=` — kept as a flat cross-class session list (useful for "everything
  ...
  schedule-type occurrences + all assigned-type sessions); it does not need to know about virtual/unmaterialized
  dates, since those aren't real rows.
...
- Attendance: mark, then correct, a student's status and confirm the row updates in place (no duplicate);
  ...
  trial student (no `class_enrollments` row) can still get an attendance row.
...
updated (the `classes` column swap, the two new tables) in the same change that implements this spec, and the
local dev database must be migrated by hand via a throwaway `pg` script, mirroring how every previous schema
change in this project's history was applied.
